...
class MattermostChannel(BaseChannel):
    ...
    async def _handle_posted_event(self, data: dict[str, Any], broadcast: dict[str, Any]) -> None:
        ...
        if is_dm:
            if not await self._is_dm_allowed(user_id):
                return
        elif not self._should_respond_in_channel(chat_id, content):
            return

        sender_id = await self._build_sender_id(user_id, data, post)
        await self._prime_identity_cache_for_allow_check(user_id)
        ...
        session_key = None
        if self.config.reply_in_thread and not is_dm:
    ...
    def is_allowed(self, sender_id: str) -> bool:
        ...
        matched = False
        if mode == "id":
            matched = user_id in allow_list if user_id else sender_str in allow_list
        elif mode == "username":
            matched = bool(username and username in allow_list)
        elif mode == "email":
            if user_id:
                email = self._user_emails.get(user_id)
                normalized_allow = {value.lower() for value in allow_list}
                matched = bool(email and email.lower() in normalized_allow)
        if matched:
            return True
        return super().is_allowed(sender_str)
    ...
    async def _prime_identity_cache_for_allow_check(self, user_id: str) -> None:
        """Populate identity fields needed by synchronous BaseChannel permission checks."""
        if self.config.allow_from_match_mode != "email":
            return
        try:
            await self._get_user_identity(user_id)
        except Exception as e:
