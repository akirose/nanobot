...
@pytest.mark.asyncio
async def test_unauthorized_dm_sends_pairing_code_without_publishing(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("nanobot.channels.base.generate_code", lambda _channel, _sender: "ABCD-EFGH")
    channel = _make_channel(
        allow_from=[],
        dm={"enabled": True, "policy": "open", "allow_from": []},
    )
    ...
    fake = _FakeAsyncClient()
    fake.post_responses.append(_FakeResponse({"id": "pairing-post"}, status_code=201))
    channel._client = fake

    await channel._handle_websocket_message(
        json.dumps(_posted_payload(user_id="unknown-user", channel_id="dm1"))
    )

    assert channel.bus.inbound_size == 0
    assert len(fake.post_calls) == 1
    assert fake.post_calls[0]["url"].endswith("/api/v4/posts")
    assert fake.post_calls[0]["json"]["channel_id"] == "dm1"
    assert "ABCD-EFGH" in fake.post_calls[0]["json"]["message"]
...
def test_is_allowed_uses_pairing_fallback_when_allowlist_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("nanobot.channels.base.is_approved", lambda channel, sender: True)
